...
/***
	Place you PERSONAL information here.
***/
...
if (root == NULL)
root = newNode; // Insert the node
else if (newNode.getInfo() < root.getInfo())
insert(root->left, newNode); // Search the left branch
else
insert(root->right, newNode);// Search the right branch
}
